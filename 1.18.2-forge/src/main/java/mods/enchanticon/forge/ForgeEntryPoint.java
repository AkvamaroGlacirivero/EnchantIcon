...
public class ForgeEntryPoint {
    ...
    public ForgeEntryPoint() {
        ...
        context.registerExtensionPoint(IExtensionPoint.DisplayTest.class,
                () -> new IExtensionPoint.DisplayTest(() -> NetworkConstants.IGNORESERVERONLY, (incomingVer, isNetwork) -> true));
        context.registerConfig(ModConfig.Type.CLIENT, ModConfigImpl.init(new ForgeConfigSpec.Builder()));

        if (ModList.get().isLoaded("cloth_config")) {
            ModLoadingContext.get().registerExtensionPoint(ConfigGuiHandler.ConfigGuiFactory.class,
                    () -> new ConfigGuiHandler.ConfigGuiFactory(ModConfigScreenImpl::create));
        }
    }
    ...
    public static final class ClientInit {
        ...
        public static void modelLoaderReg(ModelRegistryEvent event) {
            ...
            for (var registryKey : ForgeRegistries.ENCHANTMENTS.getKeys()) {
                var iconKey = new ResourceLocation("enchant_icon", "enchant/" + registryKey.getNamespace() + "/" + registryKey.getPath());
                ForgeModelBakery.addSpecialModel(iconKey);
            }
            ForgeModelBakery.addSpecialModel(Constants.ENCHANT_ICON_MODEL);
        }
